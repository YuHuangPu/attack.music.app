...
//            String sql =
//                "SELECT FORMAT(SUM(GD.PRICE), 0) AS `SUM`, GD.STATUS FROM GOODS_DETAIL GD WHERE GD.STATUS != 'N' GROUP BY GD.STATUS";
...
//                + "SELECT SUM(PRICE) AS SUM, DATE_FORMAT(SELL_DATE, '%y-%m-%d') AS SELL_DATE FROM GOODS_DETAIL GROUP BY SELL_DATE ORDER BY DATE_FORMAT(SELL_DATE, '%Y-%m-%d') DESC  LIMIT 0, 30"
